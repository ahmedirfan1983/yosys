  ...
  std::string dump_cell(bool& output_type, const RTLIL::Cell* cell, bool context)
	{
	  ...
	  if(it==std::end(expr_ref))
		...
		if(output_width != w && context) {
		  str = stringf ("__expr%d := resize(%s);", ++line_num, cell_expr.c_str());
		  f << stringf("%s\n", str.c_str());
		  cell_expr = stringf("__expr%d", line_num);		  
		}
		output_type = context;
		...
		*/
		...
		log(" - width is %d\n", output_width);
		std::string cond = dump_sigspec(t1, &cell->getPort(RTLIL::IdString("\\CLK")), 1, false);
		...
		std::string data = dump_sigspec(t1, &cell->getPort(RTLIL::IdString("\\DATA")), address_width, true);
	...
	void dump()
	{
		...
		for(auto cell_it = module->cells_.begin(); cell_it != module->cells_.end(); ++cell_it) {
		  RTLIL::Cell *cell = cell_it->second;
		  if(cell->type == "$dff" || cell->type == "$adff" || cell->type == "$dffsr" || cell->type == "$memwr") {
		    bool output_type;
		    str = dump_cell(output_type, cell_it->second, true);                                             
		    if(cell->type != "$memwr")
		...
		for(unsigned long i=0; i < trans_expr_list.size() - 1; ++i) {
